...
export default async function IndividualDashboard() {
  ...
  try {
    ...
    // ✅ এখন সঠিকভাবে currentUserId পাস করা হচ্ছে
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-6 rounded-3xl text-white shadow-lg relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="mt-4 text-xs bg-white/20 w-fit px-2 py-1 rounded-lg">
              আপনার প্রোফাইল ভ্যালু বাড়ছে
            </p>
          </div>
          <Briefcase
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between">
          ...
          <span>ভেরিফিকেশনের জন্য আবেদন করুন <ArrowRight size={14} className="inline" /></span>
          </button>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between">
          ...
          <p className="text-xs text-gray-400 mt-4">
            আপনার লোকেশন ভিত্তিক ২০ জন ইউজার আশেপাশে আছে।
          </p>
        </div>
      </div>
            ...
            {connections && connections.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {connections.slice(0, 4).map((conn: any) => (
                  <div
                    key={conn.id}
                    className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:border-blue-200 transition-all"
                  >
              ...
              <div className="text-center py-10">
                <p className="text-gray-400">
                  আপনার রুট অনুযায়ী কোনো কানেকশন পাওয়া যায়নি।
                </p>
                <Link
                  href="/root"
                  className="text-sm text-blue-500 underline mt-2 block"
                >
                  আরও প্রতিষ্ঠান যোগ করুন
                </Link>
              </div>
            )}
          </div>
